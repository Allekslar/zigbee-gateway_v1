...
| `INV-H006` | High | `docs/implementation/PRODUCTION_HARDENING_PLAN.md` (S3 INV-PERS-01: explicit versioned persistence) | `components/service` | No new code may `reinterpret_cast`/blob a raw `core::CoreState` for persistence; the explicit `to_payload()`/`apply_payload()` conversion and the two-generation `PersistedStateStore` must exist and remain the sole persistence path. |
| `INV-H007` | High | `docs/implementation/PRODUCTION_HARDENING_PLAN.md` (S4 FD-17 slice: canonical GatewayId) | `components/common/gateway_id.{hpp,cpp}`, `components/app_hal/hal_identity.c` | `GatewayId` formatting must stay ESP-IDF-free; the factory base MAC eFuse read must be owned exclusively by `hal_identity.c`. |
| `INV-H008` | High | `docs/implementation/PRODUCTION_HARDENING_PLAN.md` (S4 #20-#25: DeviceId-keyed Matter endpoint allocation) | `components/service/include/matter_endpoint_registry.hpp`, `components/matter_bridge`, `components/service/include/matter_runtime_api.hpp`, `components/app_hal/include/hal_matter.h` | `MatterEndpointRegistry` (DeviceId-keyed allocator) must exist; the Matter bridge must not contain a class-wide fixed endpoint fallback; `MatterBridgeDeviceSnapshot` must carry a resolved endpoint; `hal_matter.h` must expose a removal/tombstone primitive. |
