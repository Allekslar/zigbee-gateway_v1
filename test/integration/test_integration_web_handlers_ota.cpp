...
int main() {
    ...
    assert(!web_ui::register_ota_routes(nullptr, &route_ctx));
    assert(!web_ui::register_ota_routes(reinterpret_cast<void*>(1), nullptr));
    ...
    assert(web_ui::ota_get_handler(nullptr) == ESP_FAIL);
    assert(web_ui::ota_get_handler(&bad_req) == ESP_FAIL);
    assert(web_ui::ota_post_handler(nullptr) == ESP_FAIL);
    assert(web_ui::ota_post_handler(&bad_req) == ESP_FAIL);
    assert(web_ui::ota_result_get_handler(nullptr) == ESP_FAIL);
    assert(web_ui::ota_result_get_handler(&bad_req) == ESP_FAIL);
