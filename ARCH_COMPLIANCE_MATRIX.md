...
| `INV-M033` | Medium | `ARCHITECTURE.md` + `CODING_GUIDELINES.md` (modular service internals / async result seam extraction) | `components/service/include/operation_result_store.hpp`, `components/service/include/service_runtime.hpp`, `components/service/include/service_runtime_api.hpp`, `components/service/service_runtime.cpp` | Network operation request-id generation and result queueing must live in a dedicated `OperationResultStore`; `ServiceRuntime`/`ServiceRuntimeApi` should delegate request allocation and result storage/retrieval instead of owning them inline. |
| `INV-M034` | Medium | `ARCHITECTURE.md` + `CODING_GUIDELINES.md` (modular service internals / Zigbee lifecycle seam extraction) | `components/service/include/zigbee_lifecycle_coordinator.hpp`, `components/service/include/service_runtime.hpp`, `components/service/service_runtime.cpp` | Zigbee join-window cache, duplicate join suppression, and lifecycle policy coordination must live in a dedicated `ZigbeeLifecycleCoordinator`; `ServiceRuntime` should delegate these operations instead of owning the cache/helpers inline. |
